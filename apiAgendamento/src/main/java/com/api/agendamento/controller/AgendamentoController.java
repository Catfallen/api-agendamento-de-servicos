...
public class AgendamentoController {
    ...
    */
    @DeleteMapping("/{id}")
    public ResponseEntity<String> excluirAgendamento(@PathVariable Integer id) {
        try {
            agendamentoService.excluirAgendamento(id);
            // Retorna uma mensagem de sucesso com o status 200
            return ResponseEntity.ok("Agendamento excluído com sucesso.");
        } catch (RuntimeException e) {
            // Retorna uma mensagem de erro com o status 404
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("Agendamento não encontrado para o ID fornecido.");
        }
    }
